import pygame
from .constants import BLACK, ROWS, creamWhite, SQUARE_SIZE, COLS, WHITE
from .piece import Piece

class Board:
    def __init__(self):
        self.board = []
        self.creamWhite_left = self.white_left = 12
        self.creamWhite_kings = self.white_kings = 0
        self.createBoard()
    ...
    def drawSquares(self, win):
        ...
        for row in range(ROWS):
            for col in range(row % 2, COLS, 2):
                pygame.draw.rect(win, creamWhite, (row*SQUARE_SIZE, col *SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))

    def evaluate(self):
        return self.white_left - self.creamWhite_left + (self.white_kings * 0.5 - self.creamWhite_kings * 0.5)
            ...
            for col in range(COLS):
                if col % 2 == ((row +  1) % 2):
                    if row < 3:
                        ...
                        self.board[row].append(Piece(row, col, creamWhite))
                    else:
    ...
    def remove(self, pieces):
        for piece in pieces:
            ...
            if piece != 0:
                if piece.color == creamWhite:
                    self.creamWhite_left -= 1
                else:
                    self.white_left -= 1
    
    def winner(self):
        if self.creamWhite_left <= 0:
            return WHITE
    ...
    def getValidMoves(self, piece):
        ...
        if piece.color == creamWhite or piece.king:
